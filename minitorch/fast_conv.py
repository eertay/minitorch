...
def _tensor_conv1d(
    ...
    for out_i in prange(out_size):
        ...
        acc = 0.0
        # Iterate through in channels and the weight array
        for cur_in_channels in prange(in_channels):
            for cur_kw in range(kw):
                # Get the position in the input using stride
                var = cur_kw
                w_pos = s2[0] * cur_out_channels + s2[1] * cur_in_channels + s2[2] * var
                w = weight[w_pos]
                in_pos = 0.0
                # Backward pass
                if reverse:
                    ...
                    var = kw - cur_kw - 1
                    # Check if the window out column + width of kerbel is more than input shape
                    if cur_width - var >= 0:
                        in_pos = input[
                            cur_batch * s1[0]
                            + cur_in_channels * s1[1]
                            + (cur_width - var) * s1[2]
                        ]
                # Forward pass
                else:
                    # Find the position in the input using strides but add position the weight to create the window effect
                    if cur_width + var < width:
                        in_pos = input[
                            cur_batch * s1[0]
                            + cur_in_channels * s1[1]
                            + (cur_width + var) * s1[2]
        ...
        out_pos = (
            ...
            + cur_out_channels * out_strides[1]
...
def _tensor_conv2d(
    ...
    for out_i in prange(out_size):
            ...
            for cur_kh in range(kh):
                for cur_kw in range(kw):
                    ...
                    if reverse:
                        ...
                        if cur_height - i >= 0 and cur_width - j >= 0:
                            in_pos = input[
                                cur_batch * s10
                                + cur_in_channels * s11
                                + (cur_height - i) * s12
                        ...
                        if cur_height + i < height and cur_width + j < width:
                            in_pos = input[
                                cur_batch * s10
                                + cur_in_channels * s11
                                + (i + cur_height) * s12
        ...
        out_pos = (
            cur_batch * out_strides[0]
            + cur_out_channels * out_strides[1]
            + cur_height * out_strides[2]
            + cur_width * out_strides[3]
        )
        out[out_pos] = acc
