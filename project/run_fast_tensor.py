...
def plot_graph(corrects, losses, times):
    plt.figure(0)
    plt.plot(corrects)
    plt.title("Accuracy per 10 Epochs")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    ...
    plt.figure(2)
    plt.plot(times)
    plt.title("Average time in seconds per 10 Epochs")
    plt.xlabel("Epoch")
    plt.ylabel("Time")
...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        layer1 = self.layer1.forward(x).relu()
        layer2 = self.layer2.forward(layer1).relu()
        return self.layer3.forward(layer2).sigmoid()
    ...
    def train(self, data, learning_rate, max_epochs=500, log_fn=default_log_fn):
        ...
        for epoch in range(max_epochs):
            ...
            if epoch % 10 == 0 or epoch == max_epochs:
                ...
                avg_time = total_epoch_time / max_epochs
                times.append(avg_time)
                corrects.append(correct)
                loss_freq.append(total_loss)
        ...
        # plot_graph(corrects, loss_freq, times)
