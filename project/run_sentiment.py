...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        x1 = minitorch.nn.max(self.layer1(embeddings).relu(), 2)
        x2 = minitorch.nn.max(self.layer2(embeddings).relu(), 2)
        x3 = minitorch.nn.max(self.layer3(embeddings).relu(), 2)
        x_mid = x1 + x2 + x3
        # x = self.linear(
        #     x_mid.view(x_mid.size // self.feature_map_size, self.feature_map_size)
        # )
        x = self.linear(x_mid.view(x_mid.shape[0], self.feature_map_size))
